					...
					<div class="row">
						<div class="col-md-8 article">
							<?php if(get_field('hide_page_title', $the_ID)) { ?>
								<?php echo ( get_field('show_custom_title', $the_ID ) ? '<h2 class="article-title">' . get_field('page_title', $the_ID ) . '<span class="edit-link">' . edit_post_link('Edit this page', ' | ', '') . (( current_user_can('edit_posts') ) ? ' | <a href="' . add_query_arg(array('post_type'=>'page'),admin_url('post-new.php')) . ' title="Add new page" class="post-add-link">Add new page</a>' : '') . '</span></h2>' : '' ); ?>
							<?php } else { ?>
